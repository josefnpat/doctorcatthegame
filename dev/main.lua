require("TEsound")
TEsound.playLooping("assets/pk-strangerdanger-final.mp3", "music")
...
function love.load()
  ...
  title_font = love.graphics.newFont("assets/where stars shine the brightest.ttf",128)
  text_font = love.graphics.newFont("assets/where stars shine the brightest.ttf",64)
  key_font = love.graphics.newFont("assets/where stars shine the brightest.ttf",48)
...
function love.update(dt)
  ...
  elseif state == "game" then
    ...
    if not scamper_for_level and cat.timeleft <= 0 then
      ...
      TEsound.play("assets/level"..current_level..".wav")
    end
    ...
    for i,v in ipairs(craps) do
      ...
      if distance(cat,v)<cat.rad then
        table.remove(craps,i)
        add_coins_lost(16)
        TEsound.play("assets/coins_lost_crap.wav")
      end
    end
    ...
    for i,v in ipairs(ghostcats) do
      ...
      if distance(cat,v)<cat.rad then
        table.remove(ghostcats,i)
        add_coins_lost(4)
          TEsound.play("assets/coins_lost_ghostcat.wav")
      end
    end
